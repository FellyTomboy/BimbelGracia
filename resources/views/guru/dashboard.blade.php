    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-lg font-semibold mb-4">Kalender Les Saya</h3>
                @php
                    $now = now();
                    ...
                    $attendances = \App\Models\MonthlyAttendance::with(['enrollment.program', 'students'])
                        ->whereHas('enrollment', fn ($q) => $q->where('teacher_id', $teacher?->id ?? 0))
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <p class="mt-2 text-sm text-gray-600">Isi presensi untuk setiap sesi les. Maksimal 7 hari setelah les.</p>
                    <div class="mt-4 flex gap-3">
                        <a href="{{ route('guru.presensi.create') }}" class="px-4 py-2 rounded-md bg-slate-900 text-white text-sm">Isi Presensi</a>
                        <a href="{{ route('guru.presensi.index') }}" class="px-4 py-2 rounded-md border text-sm">Riwayat Presensi</a>
                    </div>
                </div>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
