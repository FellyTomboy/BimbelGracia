    ...
    <div class="space-y-4">
        ...
        <div class="bg-rose-50 border border-rose-200 rounded-xl p-4 space-y-2">
            <div class="flex justify-between text-sm">
                <span class="text-rose-600">Nama</span>
                <span class="font-medium text-rose-900">{{ $registrant->name }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-rose-600">No. WA</span>
                <span class="font-medium text-rose-900">{{ $registrant->whatsapp ?? '-' }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-rose-600">Tanggal Daftar</span>
                <span class="font-medium text-rose-900">{{ $registrant->created_at->format('d M Y H:i') }}</span>
            </div>
            @if ($registrant->converted)
