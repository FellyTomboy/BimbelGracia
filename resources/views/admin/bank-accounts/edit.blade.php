<x-app-layout>
    <x-slot name="title">Edit Rekening Bimbel</x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Rekening Bimbel</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.bank-accounts.update', $bankAccount) }}" class="p-6 space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nama Bank</label>
                        <input name="bank_name" value="{{ old('bank_name', $bankAccount->bank_name) }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                    ...
                    <div>
                        ...
                        <input name="account_number" value="{{ old('account_number', $bankAccount->account_number) }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                    ...
                    <div>
                        ...
                        <input name="account_holder" value="{{ old('account_holder', $bankAccount->account_holder) }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-1 w-full border-gray-300 rounded-md" required>
                            ...
                            <option value="hibernasi" @selected(old('status', $bankAccount->status) === 'hibernasi')>hibernasi</option>
                        </select>
