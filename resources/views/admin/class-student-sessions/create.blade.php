<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Jadwal Murid Kelas</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.class-student-sessions.store') }}" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="class_student_id" class="mt-1 w-full border-gray-300 rounded-md" required>
                            ...
                            @foreach ($students as $student)
                                <option value="{{ $student->id }}" @selected(old('class_student_id') == $student->id)>{{ $student->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" value="{{ old('start_time') }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                            @error('start_time')<p class="text-sm text-rose-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Jam Selesai</label>
                            <input type="time" name="end_time" value="{{ old('end_time') }}" class="mt-1 w-full border-gray-300 rounded-md" />
