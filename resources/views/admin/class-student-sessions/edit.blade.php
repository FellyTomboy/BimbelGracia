<x-app-layout>
    <x-slot name="title">Edit Presensi Kelas</x-slot>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <x-breadcrumb :items="[['label' => 'Kalender Kelas', 'url' => route('admin.class-student-sessions.index', ['month' => $month, 'year' => $year])], ['label' => 'Edit Presensi']]" />
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Presensi Kelas</h2>
                <p class="text-sm text-gray-500 mt-0.5">
                    {{ $session->session_date->format('d M Y') }} — {{ $session->program->name }}
                </p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.class-student-sessions.update', $session) }}"
                        ...
                        <div>
                            ...
                            <select name="program_id" id="program-select"
                                ...
                                @foreach ($programs as $p)
                                    <option value="{{ $p->id }}" @selected($session->program_id == $p->id)>
                                        {{ $p->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="border rounded-xl p-4 bg-gray-50 max-h-80 overflow-y-auto">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2" id="student-checkboxes">
                                ...
                                <template x-for="s in students" :key="s.student_id">
                                    <label class="flex items-center gap-2 text-sm p-2.5 bg-white rounded-lg border cursor-pointer transition-colors"
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" class="w-full rounded-xl border-gray-200 text-sm"
                                  placeholder="Opsional...">{{ old('notes', $session->notes) }}</textarea>
                    </div>
    ...
    <script>
        function classPresensiEdit() {
            return {
                ...
                selectedTeachers: @json($sessionTeachers),
                teacherSearch: '',
                ...
                init() {
                    // Build allStudentsByProgram grouped by program_id
                    ...
                    const existingStudentIds = @json($existingStudentIds);
                    const existingEnrollmentMap = @json($existingEnrollmentMap);
                    existingStudentIds.forEach(id => {
                        if (this.studentEnrollmentMap[id] === undefined) {
