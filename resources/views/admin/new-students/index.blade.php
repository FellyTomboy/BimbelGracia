                ...
                <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        @if ($newStudents->count() > 0)
                            <form method="POST" action="{{ route('admin.new-students.destroy-all') }}" onsubmit="return confirm('Hapus SEMUA data pendaftar? Tindakan ini tidak bisa dibatalkan.')">
                                @csrf
                                @method('DELETE')
                            ...
                            @forelse ($newStudents as $ns)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-2">
                                            @if (!$ns->converted)
                                                <form method="POST" action="{{ route('admin.new-students.convert', $ns) }}" onsubmit="return confirm('Tambahkan data pendaftar ini ke data murid?')">
                                                    @csrf
