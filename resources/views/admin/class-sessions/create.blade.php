<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Jadwal Kelas</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="GET" action="{{ route('admin.class-sessions.create') }}" class="p-6 grid md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Kelas</label>
                        <select name="class_group_id" class="mt-1 w-full border-gray-300 rounded-md">
                            <option value="">Pilih kelas</option>
                            @foreach ($groups as $group)
                                <option value="{{ $group->id }}" @selected(request('class_group_id') == $group->id)>{{ $group->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex items-end">
                        <button type="submit" class="px-4 py-2 rounded-md border">Tampilkan Murid</button>
                    </div>
                </form>
            </div>

            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.class-sessions.store') }}" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="class_group_id" class="mt-1 w-full border-gray-300 rounded-md" required>
                            <option value="">Pilih kelas</option>
                            @foreach ($groups as $group)
                                <option value="{{ $group->id }}" @selected(old('class_group_id', $selectedGroup?->id) == $group->id)>{{ $group->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="date" name="session_date" value="{{ old('session_date') }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                        ...
                        <div>
                            ...
                            <input type="time" name="session_time" value="{{ old('session_time') }}" class="mt-1 w-full border-gray-300 rounded-md" />
                        ...
                        <div>
                            ...
                            <input name="subject" value="{{ old('subject', $selectedGroup?->subject) }}" class="mt-1 w-full border-gray-300 rounded-md" />
                    ...
                    <div>
                        ...
                        @else
                            <p class="mt-2 text-sm text-gray-500">Pilih kelas untuk menampilkan murid.</p>
                        @endif
