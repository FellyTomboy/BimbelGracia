<form id="att-detail-form" x-on:submit.prevent="$dispatch('att-detail-submit')">
    ...
    <div class="space-y-4">
        <p class="text-sm text-gray-600">
            Ubah status validasi presensi ini.
        </p>

        <div>
            <label for="validate-status" class="block text-sm font-medium text-gray-700 mb-1">Status Validasi</label>
            <select name="status" id="validate-status" class="w-full border-gray-200 rounded-xl text-sm" required>
                <option value="terima">Terima</option>
                <option value="terlambat">Terlambat (potongan 10%)</option>
