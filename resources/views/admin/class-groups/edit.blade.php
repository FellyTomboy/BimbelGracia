<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Kelas Bersama</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.class-groups.update', $classGroup) }}" class="p-6 space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nama Kelas</label>
                        <input name="name" value="{{ old('name', $classGroup->name) }}" class="mt-1 w-full border-gray-300 rounded-md" required />
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="subject" value="{{ old('subject', $classGroup->subject) }}" class="mt-1 w-full border-gray-300 rounded-md" />
                        ...
                        <div>
                            ...
                            <select name="teacher_id" class="mt-1 w-full border-gray-300 rounded-md" required>
                                @foreach ($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" @selected(old('teacher_id', $classGroup->teacher_id) == $teacher->id)>{{ $teacher->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="notes" class="mt-1 w-full border-gray-300 rounded-md">{{ old('notes', $classGroup->notes) }}</textarea>
                            ...
                            @foreach ($students as $student)
                                <label class="flex items-center gap-2 text-sm">
                                    <input type="checkbox" name="student_ids[]" value="{{ $student->id }}"
                                        @checked(in_array($student->id, old('student_ids', $classGroup->students->pluck('id')->all()))) />
