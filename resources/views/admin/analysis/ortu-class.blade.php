<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">WA Ortu Kelas</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <form method="GET" action="{{ route('admin.analysis.ortu-kelas') }}" class="p-6 grid md:grid-cols-3 gap-4">
                    <div>
                            ...
                            <div class="text-sm text-gray-700 space-y-2">
                                @foreach ($summary['students'] as $studentSummary)
                                    <div class="border rounded-md p-3 space-y-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.analysis.ortu-class-discount') }}" class="grid gap-2 sm:grid-cols-3">
                                            @csrf
                                            <input type="hidden" name="month" value="{{ $month }}" />
                                            <input type="hidden" name="year" value="{{ $year }}" />
                                            <input type="hidden" name="class_student_id" value="{{ $studentSummary['class_student_id'] }}" />
                                            <select name="discount_type" class="w-full border-gray-300 rounded-md text-sm">
                                                @php $discountType = $studentSummary['discount']['type'] ?? 'none'; @endphp
                                            ...
                                            <input type="number" name="discount_value" min="0" value="{{ $studentSummary['discount']['value'] ?? '' }}" placeholder="Nilai" class="w-full border-gray-300 rounded-md text-sm" />
                                            <button type="submit" class="px-3 py-1 rounded-md bg-slate-900 text-white text-sm">Simpan</button>
                                        </form>
                                        @if (($studentSummary['discount']['type'] ?? null) !== null)
                                            <div class="flex justify-between text-xs text-emerald-700">
                                                <span>Diskon</span>
                                                <span>{{ $studentSummary['discount']['label'] }}</span>
                                            </div>
                                            <div class="flex justify-between text-xs font-semibold">
                                                <span>Total setelah diskon</span>
                                                <span>{{ number_format($studentSummary['discount']['total']) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
