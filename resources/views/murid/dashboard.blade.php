<x-app-layout>
    <x-slot name="title">Dashboard Murid</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Dashboard Murid</h2>
            <p class="text-sm text-gray-500 mt-0.5">Selamat datang, {{ auth()->user()?->name ?? 'Murid' }}</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            @php
                ...
                $month = now()->month;
                $year = now()->year;
                $totalLes = \App\Models\MonthlyAttendance::whereHas('students', fn($q) => $q->where('student_id', $studentId))
                    ->whereMonth('lesson_date', $month)->whereYear('lesson_date', $year)->count();
                $totalGuru = \App\Models\Enrollment::where('student_id', $studentId)->where('status', 'active')->count();
                $kpis = [
                    ['icon' => '📅', 'label' => 'Les Bulan Ini', 'value' => $totalLes, 'color' => 'from-blue-500 to-blue-600'],
                    ['icon' => '👨‍🏫', 'label' => 'Total Guru', 'value' => $totalGuru, 'color' => 'from-emerald-500 to-emerald-600'],
