/**
 * Alpine.js New Student Modal Factory
 *
 * Handles convert + delete actions for new student registrations via AJAX + modal.
 *
 * Usage in Blade:
...
export function newStudentModal(config) {
    return {
        modalOpen: false,
        modalTitle: '',
        modalBody: '',
        ...
        previewConvertUrl: config.previewConvertUrl || ((id) => `/admin/new-students/${id}/preview-convert`),
        convertUrl: config.convertUrl || ((id) => `/admin/new-students/${id}/convert`),
        deleteUrl: config.deleteUrl || ((id) => `/admin/new-students/${id}`),
        deleteAllUrl: config.deleteAllUrl || '/admin/new-students/all',
        ...
        async openConvertModal(id) {
            ...
            try {
                ...
                this.$nextTick(() => {
                    const form = document.getElementById('ns-modal-form');
        ...
        async openDeleteAllModal() {
            ...
            try {
                const response = await window.Ajax.get('/admin/new-students/preview-delete-all');
                this.modalBody = response.data.html || '';
                this.$nextTick(() => {
                    const form = document.getElementById('ns-modal-form');
                    if (form && window.Alpine) {
        ...
        async _doConvert() {
            ...
            try {
                ...
                window.Toast?.success('Berhasil ditambahkan ke data murid.');
                this.close();
        ...
        async _doDeleteAll() {
            ...
            try {
                ...
                window.Toast?.success('Semua data pendaftar dihapus.');
                this.close();
                await this.refreshTable();
            } catch (e) {
